(function () {
    ...
    function createSearchTextIndex(value) {
        ...
        // Mapeamento de volta para o texto original para obter snippets ancorados nos matches normalizados.
    ...
    // Função de realce (highlight) segura contra XSS
    function highlightText(text, tokens) {
        ...
        // Escapa caracteres de regex nos tokens do usuário
        ...
        // Substituição case-insensitive dos tokens
    ...
    function renderResults(entries, query) {
        ...
        if (!tokens.length) {
            renderEmptyState("Digite para buscar posts por título, tag ou conteúdo.", "search-status-empty");
            return;
        }

        if (!entries.length) {
            renderEmptyState("Nenhum post encontrado para \"" + query + "\".", "search-status-empty");
            return;
        }

        status.textContent = entries.length + (entries.length === 1 ? " resultado encontrado" : " resultados encontrados") + " para \"" + query + "\"";
        ...
        entries.forEach(function (entry) {
            ...
            link.innerHTML = highlightText(entry.title, tokens);
            ...
            if (entry.section) {
                var sectionTranslated = entry.section.toLowerCase() === "posts" ? "Posts" : entry.section;
                meta.textContent += " em " + sectionTranslated;
            }
            ...
            if (entry.tags.length) {
                ...
                entry.tags.forEach(function (tag) {
                    ...
                    var tagLabel = createElement("span", "search-tag");
                    tagLabel.innerHTML = highlightText(tag, tokens);
            ...
            // Snippet/Trecho
            var snippetText = getSnippet(entry, tokens);
            if (snippetText) {
                var snippet = createElement("p", "search-result-snippet");
    ...
    if (!indexUrl) {
        renderEmptyState("Índice de busca indisponível. Adicione `outputs = [\"html\", \"json\"]` ao front matter do post.", "search-status-error");
        return;
    }
    ...
    fetch(indexUrl)
        ...
        .catch(function () {
            renderEmptyState("Não foi possível carregar o índice de busca.", "search-status-error");
        });
}());
